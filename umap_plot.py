...
from umap import UMAP  # <-- use UMAP
...
def main(animation=False, model_type="ssl", weight=None, interval=1000):
    ...
    if not animation:
        ...
        plt.xlabel("Dim 1")
        plt.ylabel("Dim 2")
        plt.title(f"UMAP Embeddings ({weight})")
        plt.savefig(os.path.join(model_type, f"umap_{weight}.png"))
        # plt.show()
        ...
        for w in weight_files:
            ...
            # Compute UMAP independently per model
            umap_proj = UMAP(n_components=2, random_state=42)
        ...
        title_text = ax.set_title(f"UMAP Embeddings ({weight_files[0]})")
        plt.colorbar(scatter)
        plt.xlabel("Dim 1")
        plt.ylabel("Dim 2")
        ...
        anim = FuncAnimation(
            fig, update, frames=len(all_emb_2d), interval=1000, blit=True
        )
